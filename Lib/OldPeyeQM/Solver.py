...
def Schroedinger(File, Nodes = 0, Elems = 0, parameter = [], Dimension = 1, \
                 BCType = 'Dir', Type = 'Stationary', Eq = 'Schro', \
                 AnalisisParam = ['y', 'y', 4, 4, 101]):
    """
        ...
        parameter:  Is an array that describes the potential actuating over the
                    the elements of the domain given by Elems. For each element 
                    in Elems there is an associated potential value on the same
                    ...
                    to define a potential and what does it means read the
        ...
        AnalisisParam:   Array that contains the information regarding the 
                        ...
                        AnalisisParam[1]:  String  answer to the question
                                                   save  Eigen Vectors?
                        AnalisisParam[2]:  Integer number of Eigen Values 
                                                   to save
                        AnalisisParam[3]:  Integer number of Eigen Vectors 
                                                   to save


	Last modification: date 1/11/2011
    """
    ...
    if File != '':      # If type is not blank
       ...
       if Dimension == 1:
            ...
            Nodes, eLines, eTria = Readmsh(File)  # Import nodes lines and elements
            nTr = shape(eTria)[0]     #Number of Triangle elements
    ...
    #------------------------------  2D problem -------------------------------
        ...
        if 'Dir' in BCType and 'Stationary' in Type:
            ...
            K,M = matAssembly2D(Nodes, eTria, N, nTr, V)
...
def fn_matAssembly2D(Nodes, eTria, N, nEl, V):
    """
        Assembly the equivalent stiffness and equivalent mass matrices for a
        2D mesh.
